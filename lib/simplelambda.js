...
function Variable(name) {
  // if (!isLetter(name)) throw "invalid variable name"; I removed this so that I could input numbers
  ...
  this.substitute = function (varname, newterm) {
    if (name === varname) return newterm;
...
function Lambda(name, term) {
  ...
  this.substitute = function (varname, newterm) {
    ...
    if (name !== varname && newterm.hasFree(name)) {
      ...
      var term2 = term
        .substitute(name, new Variable(newname))
        .substitute(varname, newterm);
    ...
    if (name !== varname)
      if (body === term) return this;
      else return new Lambda(name, body);
...
function Apply(left, right) {
  this.toString = function () {
    ...
    if (
      Apply.prototype.isPrototypeOf(right) ||
      Lambda.prototype.isPrototypeOf(right)
    )
      rightstr = "(" + rightstr + ")";

    if (Lambda.prototype.isPrototypeOf(left)) leftstr = "(" + leftstr + ")";
  ...
  this.reduce = function () {
    ...
    if (lreduce) return new Apply(lreduce, right);

    var rreduce = reduce(right);

    if (rreduce) return new Apply(left, rreduce);

    if (!Lambda.prototype.isPrototypeOf(left)) return null;
...
function reduce(term) {
  if (term.reduce) return term.reduce();
...
function parse(text) {
  try {
    ...
    function parseTerms(closed) {
      ...
      for (
        var newterm = parseTerm(closed);
        newterm;
        newterm = parseTerm(closed)
      )
        if (term) term = new Apply(term, newterm);
        else term = newterm;
    ...
    function parseTerm(closed) {
      ...
      if (isLetter(ch)) return new Variable(ch);
      if (!isNaN(ch)) return new Variable(ch);

      if (ch === "\\") {
        var name = text[p++];

        if (!isLetter(name)) throw "Invalid argument name";

        if (text[p++] != ".") throw "Expected '.'";
